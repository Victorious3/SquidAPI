	...
	public void onItemJoinWorld(EntityJoinWorldEvent event) {
		if (event.entity instanceof EntityItem) {
			if (ExplosionRecipe.recipes.containsKey(((EntityItem) event.entity).getEntityItem().getItem())) {
				event.world.newExplosion(event.entity, event.entity.posX, event.entity.posY, event.entity.posZ, 2, false, true);
				ItemStack item = ExplosionRecipe.recipes.get(((EntityItem) event.entity).getEntityItem().getItem());
